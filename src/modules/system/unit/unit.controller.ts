...
export class UnitController {
  ...
  async create(@Req() req: any) {
    const { user, body } = req;
    const data = await this.unitService.create(user, body);
    if (data == null) {
  ...
  async clear(@Req() req) {
    const { user, body } = req;
    const data = await this.unitService.clear();
  ...
  async remove(@Param('id') id: string) {
    ...
    if (isNumber) {
      ...
      const data = await this.unitService.delete(ids);
      if (data.affected != idsLen) {
  ...
  async update(@Req() req: any) {
    const { user, body } = req;
    const data = await this.unitService.update(body);
    if (data == null) {
  ...
  async findById(@Param('id') id: string, @Query() query: any) {
    const isNumber = /^\d+$/.test(id);
    if (isNumber) {
      const data = await this.unitService.findById(+id);
      if (data == null) {
        return { code: 1000, msg: '暂无数据' };
      }
      return { code: 200, msg: '操作成功', data: data };
    } else if (id == 'list') {
      const data = await this.unitService.list(query);
      if (data == null) {
        return { code: 1000, msg: '暂无数据' };
      }
      return { code: 200, msg: '操作成功', rows: data[0], total: data[1] };
    } else if (id == 'optionselect') {
      const data = await this.unitService.optionselect();
      if (data == null) {
        return { code: 1000, msg: '暂无数据' };
      }
      return { code: 200, msg: '操作成功', data: data };
    } else if (id == 'listIndex') {
      const data = await this.unitService.listIndex(query);
      if (data == null) {
        return { code: 1000, msg: '暂无数据' };
      }
      return { code: 200, msg: '操作成功', data: data };
